...
function PANEL:RebuildControls()
	...
	local info = PartCtrl_ProcessedPCFs[ent:GetPCF()][ent:GetParticleName()].info
	if info then
	...
	rpnl:DockMargin(0,-1,0,0) //fix the 1px of blank white space between the header and the contents]]
		...
		local val = ent:GetLoopMode() or 1
		if val == 0 then
		...
		if !ent.utilfx then drop.Combo:AddChoice(loopmode1, 1) end //utilfx don't support this mode because we don't have a way to detect when they've ended
		...
		if !IsValid(ent) or !ent.utilfx then //this option doesn't do anything for utilfx, so don't show it
	...
	for k, v in SortedPairs (ent.ParticleInfo) do
		...
		function pnl.RebuildContents(v2)
			...
			local ent2 = v2.ent
			if IsValid(ent2) and IsValid(ent2.AttachedEntity) then ent2 = ent2.AttachedEntity end
			...
			if v2.mode == PARTCTRL_CPOINT_MODE_POSITION then
				if IsValid(ent2) then
					...
					if ent2:GetClass() == "ent_partctrl_grip" then
						...
						button:SetText("Detach from model (" .. string.GetFileFromFilename(ent2:GetModel()) .. ")")
						button:SizeToContents()
						button.DoClick = function()
							ent:DoInput("cpoint_position_ent_detach", k)
						end

						local attachcount = 0
						local tab = ent2:GetAttachments()
					...
					function slider.OnValueChanged(_, val)
						ent:DoInput("cpoint_position_pos", k, 1, val)
					end
					...
					function slider.OnValueChanged(_, val)
						ent:DoInput("cpoint_position_pos", k, 2, val)
					end
					...
					function slider.OnValueChanged(_, val)
						ent:DoInput("cpoint_position_pos", k, 3, val)
					end
				...
				if istable(tab) then
					...
					if tab.label == "Color" then
						...
						function col.ValueChanged(_, val)
							...
							ent:DoInput("cpoint_vector_val_all", k, vec)
						end
					else
						for i = 1, 3 do
							local slider = vgui.Create("DNumSlider", pnl)
							...
							function slider.OnValueChanged(_, val)
								...
								ent:DoInput("cpoint_vector_val_axis", k, i, val)
							end
						end
					end
				...
				for i = 1, 3 do
					...
					if istable(tab) then
						...
						if tab.dropdown then
							...
							function drop.Combo.OnSelect(_, index, value, data)
								ent:DoInput("cpoint_axis_val", k, i, data)
							end
						...
						elseif tab.checkboxes then
							...
							for checkk, checkv in SortedPairs (tab.checkboxes) do
								...
								check.OnChange = function(_, val)
									...
									ent:DoInput("cpoint_axis_val", k, i, total)
								end
							end
						else
							local slider = vgui.Create("DNumSlider", pnl)
							...
							function slider.OnValueChanged(_, val)
								...
								ent:DoInput("cpoint_axis_val", k, i, val)
							end
						end
